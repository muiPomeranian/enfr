...
class Translator(object):
    ...
    def translate_batch(self, src_seq, src_pos):
        ...
        def get_inst_idx_to_tensor_position_map(inst_idx_list):
            ...
            return {inst_idx: tensor_position for tensor_position, inst_idx
                    in enumerate(inst_idx_list)}
        ...
        def beam_decode_step(
            ...
            word_probs.append(word_prob.cpu().data.numpy())
            # Update the beam with predicted word prob information and collect incomplete instances
            active_inst_idx_list = collect_active_inst_idx_list(
                inst_dec_beams, word_prob, inst_idx_to_position_map)

            return active_inst_idx_list, word_probs
        ...
        with torch.no_grad():
            ...
            word_probs = []  # initial container for collecting words_probs
            #-- Decode
            for len_dec_seq in range(1, self.model_opt.max_token_seq_len + 1):

                active_inst_idx_list, word_probs = beam_decode_step(
                    word_probs,
                    inst_dec_beams,
                    len_dec_seq,
                    src_seq,
